...
int findPin(uint16_t GPIO_Pin){
	...
	HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_5);
		btn_state = HAL_GPIO_ReadPin(GPIOC, GPIO_Pin);
		if(!btn_state)
			return 2;
...
// Для этого просто считаем ее состояние.
	...
	uint16_t displayNum = 4*res + findPin(GPIO_Pin);
...
// прерывания если при работе с пинами мы сновасформировали
// импульс, который инициировал данное прерывание снова.
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_NVIC_SetPriority(EXTI2_TSC_IRQn, 0, 3);
